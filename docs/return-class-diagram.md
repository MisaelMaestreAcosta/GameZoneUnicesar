...
direction TB
    ...
    class return {
	    ...
	    + return(String,Sale)
	    + double calculateRefundAmount(List<Products>)
	    + String generateReturnReceipt(String,Localdate, List>)
    }
    ...
    class ReturnService {
	    ...
	    + int registerRerturn(saleId, Productlds, reason)
	    + List  viewAllReturns() : List
	    + List  viewReturnsByCustomer(customerId) : List
	    + List  viewReturnsBySale(saleID) : List
	    + double generateMonthlyBalance(month, year)
    }
